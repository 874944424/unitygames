...
public class PlayerAnimation : MonoBehaviour
{
    ...
    void Update()
    {
        if (player.hp <= 0)
        {
            ...
            {
                networkview.RPC("PlayState", RPCMode.AllBuffered, AnimsName.soldierDie_animName);
        ...
        else
        {
            if (characterController.isGrounded == false)
            {
                ...
                networkview.RPC("PlayState", RPCMode.AllBuffered, AnimsName.soldierFalling_animName);  //第三个为参数
            }
            else
            {
                ...
                if (Mathf.Abs(h) > 0.05f || Mathf.Abs(v) > 0.05f)
                {
                    ...
                    networkview.RPC("PlayState", RPCMode.AllBuffered, AnimsName.soldierRun_animName);
                }
                else
                {
                    //PlayState(AnimsName.soldierIdle_animName);
                    networkview.RPC("PlayState", RPCMode.AllBuffered, AnimsName.soldierIdle_animName);
                }
            }
        }
    }
